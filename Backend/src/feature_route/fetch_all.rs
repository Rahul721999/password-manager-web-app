...
pub async fn fetch_all(db: web::Data<PgPool>, mid: MyMiddleware) -> Result<HttpResponse, AppError> {
    // 1. Extract Data from the token..
    let user_id = mid.user_id;

    // 2. fetch the data from the db..

    let rows = match sqlx::query_as::<_, FetchedData>(
        "SELECT id, website_name, website_url, username, password_hash 
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            return Err(AppError::InternalServerError(
                "Sorry, no data could be found in the Database".to_string(),
            ));
        }
    };

    // 3. create a VEC to store all the rows..
    let result_vec: Vec<Data> = rows
        .into_iter()
        .map(|fetched_data| {
            ...
            async move {
                ...
                Ok(Data {
                    id: fetched_data.id,
                    website_name: fetched_data.website_name,
